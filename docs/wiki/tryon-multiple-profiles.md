...
전신 생성 중인 대상은 `tryOnMakingSubject`로만 진행 화면을 표시한다. 다른 대상에 이미 전신 캐시가 있으면 즉시 그 결과를 사용할 수 있고, 대상을 되돌리면 진행 중인 작업의 최신 단계가 다시 보인다. 생성 요청은 대상 전환이나 시트 닫힘으로 취소하지 않는다. 본인 외 편집 아이콘은 모바일에서 타이틀 행 상단에 고정한다.
