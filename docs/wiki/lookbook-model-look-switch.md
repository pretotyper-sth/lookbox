...
룩북의 정사각 썸네일은 AI 착장 원본을 `contain`으로 표시해 머리와 신발까지 보인다. 카드 바탕은 원본 왼쪽 위의 배경색을 읽어 채우므로 이미지 양옆이 카드 색과 섞이지 않는다. 캔버스 읽기가 차단된 원격 이미지에는 중립 회색 폴백을 쓴다. 상세와 오늘 추천 카드도 전체 이미지를 보존한다.
