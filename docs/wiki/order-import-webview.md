...
웹스토어 공개 전 UX 검수는 개발 서버 또는 `?orderDemo=1`에서 가짜 확장 모드를 쓴다. 「샘플 주문내역 보기」 뒤에 자동 진행하지 않고 「로그인 완료 → 주문내역 열기 → 옷 가져오기」를 각각 눌러 실제 확장 사용 순서를 확인한다. 수집 중에는 찾은 옷 개수가 하나씩 늘고, 이후 「취소 / N개 담기」, 한 벌씩 상세입력, 완료 토스트까지 실제 흐름과 같지만 서버를 호출하거나 옷장에 저장하지 않는다. 실제 확장을 로컬에서 확인할 때는 `?orderReal=1`로 가짜 모드를 끈다.
