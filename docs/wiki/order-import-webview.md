...
웹스토어 공개 전 UX 검수는 개발 서버 또는 `?orderDemo=1`에서 가짜 확장 모드를 쓴다. 「샘플 주문내역 보기」를 누르면 추가 시트가 PC 2열로 넓어지고, 왼쪽에는 연결 단계, 오른쪽에는 주소 표시줄을 포함한 가상 Chrome 로그인 화면이 열린다. 가상 로그인 버튼 → 「주문내역 열기」 → 「옷 가져오기」 순서로 직접 진행한다. 오른쪽 화면은 로그인 완료·쇼핑몰 주문내역·상품 읽기 상태로 바뀌며, 수집 중에는 찾은 옷 개수가 하나씩 늘어난다. 이후 「취소 / N개 담기」, 한 벌씩 상세입력, 완료 토스트까지 실제 흐름과 같지만 서버를 호출하거나 옷장에 저장하지 않는다. 실제 확장을 로컬에서 확인할 때는 `?orderReal=1`로 가짜 모드를 끈다.
