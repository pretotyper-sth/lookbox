...
- [tryon-setup-from-mypage](tryon-setup-from-mypage.md) — 바로 보기: 대상 선택 행만큼 프사 스테이지를 144px로 줄여 사진·URL과 전체 높이 통일. tryon14는 얼굴 신원과 전신 비율을 분리하고 좌표 기반 상·하의 투명 마스크를 사용한다. 실패는 일 3회
- [tryon-multiple-profiles](tryon-multiple-profiles.md) — 바로보기에서 본인·본인 외 프로필을 저장하고 카메라 우상단에서 즉시 전환; 대상별 생성 진행 상태와 본인 외 입력 시트 위치까지 보존
