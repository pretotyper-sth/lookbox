...
레일은 `display:flex` + `overflowX:auto`이고 카드는 `flex:1 0 148px` + `maxWidth:188px`다.
148px 고정으로 두면 예전 격자보다 카드가 눈에 띄게 작아진다 — 격자의 `1fr`이 칸을
늘려 코디 4개일 때 한 칸이 약 187px였기 때문이다(2026-09-02 사용자 지적). 상한 188px은
`minmax(148px, 1fr)` 격자가 한 칸에 줄 수 있던 최대 폭이라, 코디가 둘뿐일 때 카드가
혼자 커지는 것도 막는다.
