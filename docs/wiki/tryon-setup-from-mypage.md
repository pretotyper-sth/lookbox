...
상의·하의 구간의 옷 픽셀만 투명, 양옆은 `#F2F1EE`로 막아 카메라가 비치지 않게 한다.
후면 카메라만 쓴다. 전환 버튼은 없다(2026-08-30).
전신 생성은 `gpt-image-1` + `input_fidelity=high`, 캐시 `tryon2-`(얼굴 고정 프롬프트).
OpenAI가 나가는 구간은 `POST /api/live/tryon/body`의 `images.edit`뿐. 오늘 실측
$0.108(medium 1024×1536, 2026-08-30). 카메라·구멍(`punchBody`)은 기기, $0.
