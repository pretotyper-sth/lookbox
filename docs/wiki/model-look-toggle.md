...
그 코디를 입힌 전신 컷을 만든다. `내 얼굴·체형에 맞춰 보기`가 켜진 계정만
프로필 사진·키·몸무게를 쓴다.
...
오늘 입기는 하루 한 코디만 남긴다. 다른 코디를 고르면 이전 코디는 기본 「오늘 입기」로 돌아간다(2026-09-14).
오늘 카드·상세 레일 제목은 자리 번호 `추천 코디 N`(GPT 카피 쓰지 않음).
오늘 탭 페이지 헤더는 옷장·룩북·마이페이지와 같은 좌상단 `h1` 크기로
`오늘의 추천 코디`를 고정해 보이며, 옷장 수·추천 수 설명 문구는 붙이지 않는다(2026-09-14).
모바일에서는 다시 받기를 제목 오른쪽에 유지하고, PC에서는 날짜·날씨 칩과 같은 행의 오른쪽에 둔다(2026-09-14).
...
첫 추천은 서버가 외부 아이템 설정값에 맞춰 wish 코디를 끝에 배치한다. 추가 추천은
새 카드를 뒤에 붙이고 이미 보이던 코디 순서를 다시 정렬하지 않는다.
...
규칙은 없다. 다리가 패션 일러스트처럼 늘어나면 인심이 전신의 약 2%만 짧게
보이도록 보정한다(얼굴·옷·스튜디오는 그대로, 2026-09-15).
...
직접 생성은 자동 요청의 실패 기록을 무시하고 다시 시도한다. 자동 생성이 실패한 뒤
동일 코디를 수동으로 생성할 때 `lookFailed`가 요청을 차단하던 회귀를 막는다.
직접 생성은 서버의 자동 생성 테스트 장수 제한도 우회한다.
오늘 카드의 직접 생성 callback은 코디 객체 하나를, 상세에서는 코디 배열을 전달한다.
`applyModelLooks`는 두 입력을 모두 배열로 정규화해 처리한다.
