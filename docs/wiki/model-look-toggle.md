...
Image 2+로 붙인다. 캐시 `model-id-v11-` / `model-id25-`(2026-09-14). 프롬프트는
...
메운다. 생성 픽셀에는 배경 평탄화·노이즈 보정을 다시 적용하지 않는다. Image 1의
스튜디오와 조명을 잠그고, 흰색·단색 배경을 요구하지 않는다. 생성본의 곧은 가로
경계선만 배경 픽셀에서 보간한다. 이미지의 질감과 색이
망가지는 것보다 프롬프트와 재생성으로 깨끗한 스튜디오를 받는 편이 안전하다. 우측
레일도 4:5 비율을 유지해 전신을 자르지 않는다(`_crop_look_to_card`, 2026-09-11).
