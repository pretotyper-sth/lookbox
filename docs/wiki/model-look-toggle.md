...
Image 2+로 붙인다. 캐시 `model-id-v11-` / `model-id22-`(2026-09-11). 프롬프트는
...
메운다. 생성 픽셀에는 배경 평탄화·노이즈 보정을 다시 적용하지 않는다. 이미지의 질감과
색이 망가지는 것보다 프롬프트와 재생성으로 깨끗한 스튜디오를 받는 편이 안전하다. 우측
