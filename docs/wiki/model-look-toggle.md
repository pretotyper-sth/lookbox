...
Image 2+로 붙인다. 캐시 `model-id-v11-` / `model-id17-`(2026-09-10). 프롬프트는
...
`shown` 순번이 정본이다.
「추가로 코디 추천받기」 스켈레톤 2칸은 그 버튼을 눌렀을 때만(`loading && dailyLoading`).
...
후처리는 4:5 자르기뿐이다(`_crop_look_to_card`). 인물이 창보다 크면 머리를 남긴다.
