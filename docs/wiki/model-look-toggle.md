...
상품컷과 AI 착장 모두 순백색(`#FFFFFF`) 바탕을 쓴다. AI 생성 프롬프트가 벽·바닥
전환과 접지선을 금지하고, 저장 전 줄 양끝의 배경색과 같은 픽셀을 흰색으로 정리해
벽·바닥 실선이나 두 배경이 섞이지 않게 한다(2026-09-11).
...
Image 2+로 붙인다. 캐시 `model-id-v11-` / `model-id19-`(2026-09-11). 프롬프트는
