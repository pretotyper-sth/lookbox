...
Image 2+로 붙인다. 캐시 `model-id-v11-` / `model-id16-`(2026-09-06). 프롬프트는
...
붙인다. 오늘 카드·상세 레일 제목은 자리 번호 `추천 코디 N`(GPT 카피 쓰지 않음).
...
위아래 18%는 빈 스튜디오(4:5 크롭용, `_LOOK_CROP_PAD` 0.12). 하의는 긴 기장이 기본.
후처리는 4:5 자르기뿐이다(`_crop_look_to_card`). 인물이 창보다 크면 원본에서
4:5 창을 잡아 스튜디오를 그대로 쓴다. 카드는 cover + 좌우 9% 크롭.
