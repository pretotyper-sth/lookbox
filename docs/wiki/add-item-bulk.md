...
사진·URL 탭 스테이지는 168px, 그 아래 추출 힌트 줄 44px. 바로 보기·구매내역 박스는 힌트 줄 높이까지 합쳐 212px라 탭을 바꿔도 시트 높이가 같다. 힌트 줄은 바로 보기·구매내역에서 렌더하지 않는다. 조합 추천받기(고민 중)는 CTA 아래 빈 힌트 칸을 넣지 않는다. 입력 단계는 본문 아래 패딩 12px, BottomSheet는 safe-area만 (`tightBottom`).
