...
구매내역 탭 안내: PC는 「쇼핑몰을 고른 뒤 로그인 화면이 열려요.」, 모바일 웹은 픽토그램 없이 「구매내역은 PC에서 불러올 수 있어요」 문구와 비활성 CTA만 보여준다. 구매내역은 잠금 기능이 아니라 기기 제약이므로 탭·CTA에는 `bag` 아이콘을 쓴다. 수집 목록은 「취소 / N개 담기」로 행동을 분명히 하고, 다른 소스 탭을 다녀와도 유지한다. 상세 확인에서 뒤로 돌아오면 완료된 수집 상태를 비우고 처음 쇼핑몰 선택 화면을 복원한다. 「다른 쇼핑몰」에서도 현재 구매내역을 비우고 몰 선택으로 돌아간다. 흐름은 [[order-import-webview]].
