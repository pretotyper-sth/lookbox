...
#   AI 착장 이미지        gpt-image-2 high ~$0.35              → 10
#
...
def _record_recommendation_timing(
    user_id: str | None,
    pool_size: int,
    combo_count: int,
    duration_ms: float,
    ...
    print(
        f"[timing] recommend pool={pool_size} combos={combo_count} duration_ms={ms} ({ms / 1000:.1f}s){extra}",
        flush=True,
    )
...
_STYLE_IDS = (
    ...
    "blockcore", "bodyfit", "bizcasual", "girlish", "glam", "feminine",
)
...
형식:
{
  ...
  "style": {
    ...
    "formality": 1,
    "styles": ["dandy|minimal|casual|office|street|chic|sporty|classic|amekaji|gorpcore|hiphop|y2k|preppy|blockcore|bodyfit|bizcasual|girlish|glam|feminine 중 이 옷이 실제로 잘 어울리는 무드만 1~3개"],
...
def _style_tone(style: str) -> str:
    style_map = {
        ...
        "y2k": "과감한 Y2K 톤",
        "preppy": "단정한 프레피 톤",
        "blockcore": "져지와 와이드의 블록코어 톤",
        "bodyfit": "몸을 살리는 슬림 애슬레저 톤",
        "bizcasual": "단정한 비즈니스 캐주얼 톤",
        "girlish": "사랑스러운 걸리시 톤",
        "glam": "과감한 섹시·글램 톤",
        "feminine": "부드럽고 여성스러운 페미닌 톤",
    }
...
- styles: 그 아이템이 실제로 어울리는 무드만 1~3개, 아이템마다 다르게 판단할 것.
  슬랙스·코트·로퍼 → office/classic/dandy/bizcasual, 데님·티셔츠 → casual/minimal,
  스니커·후디·카고 → street/casual/sporty, 니트·셔츠 → preppy/dandy,
  축구 져지·와이드 → blockcore, 크롭·레깅스·슬림 티 → bodyfit,
  러블리·리본 → girlish, 과감한 노출·광택 → glam, 플로럴·쉬폰 → feminine 처럼 성격에 맞게.
  전부 minimal로 채우지 말 것."""
...
def _clean_wish(raw: Any) -> dict[str, Any] | None:
    ...
    return {
        ...
        **({
            k: str(raw[k]).strip()
            for k in ("image_url", "storage_path", "thumb_url")
...
def _combo_has_shoes(
    ...
    if _combo_has_category(ids, by_id, ("shoes", "신발")):
        return True
    return bool(wish) and _item_bucket(wish) == "shoes"


def _combo_is_wearable(
    ids: list[str], by_id: dict[str, Any], wish: dict[str, Any] | None = None
) -> bool:
...
def _is_accent(item: dict[str, Any] | None) -> bool:
    ...
    return cat in (
        "bag", "가방", "hat", "모자", "misc", "기타",
        "outer", "아우터", "accessory", "액세서리", "소품",
    )
...
def _apply_wish_slot(combo: dict[str, Any], by_id: dict[str, Any]) -> None:
    """wish가 이미 있는 자리면: 상의·하의·신발은 옷장을 지키고 wish를 다른 자리로,
    가방·모자 같은 소품만 옷장 쪽을 뺀다."""
    wish = combo.get("wish")
    ...
    if not overlap:
        return
    if slot in ("shoes", "top", "bottom", "dress"):
...
def _pin_wishes_to_tail(
    ...
    for combo in combos:
        wish = combo.pop("wish", None)
        if isinstance(wish, dict):
            collected.append(wish)
...
def _model_look_composite(reference_png: bytes, board_png: bytes) -> bytes:
    """상단=옷 스트립, 아래=기준 모델. 격자 잔상이 덜 남게 모델을 크게 둔다."""
    board = Image.open(io.BytesIO(board_png)).convert("RGB")
    if board.size != (1024, 1536):
        canvas = Image.new("RGB", (1024, 1536), _LOOK_PLATE_RGB)
        strip = board.copy()
        strip.thumbnail((1024, 220))
        canvas.paste(strip, ((1024 - strip.width) // 2, 16))
    else:
        canvas = board
    ref = Image.open(io.BytesIO(reference_png)).convert("RGB")
    ref.thumbnail((780, 1180))
    canvas.paste(ref, ((1024 - ref.width) // 2, 280))
    buf = io.BytesIO()
    canvas.save(buf, format="PNG")
    return buf.getvalue()
...
def _model_look_board(items: list[dict[str, Any]]) -> bytes:
    ...
    for i, item in enumerate(items[:4]):
        try:
            ...
            continue
        image.thumbnail((slot_w - 24, 200))
        x = i * slot_w + (slot_w - image.width) // 2
        board.paste(image, (x, 20), image)
    buf = io.BytesIO()
    board.save(buf, format="PNG")
    return buf.getvalue()
...
def generate_model_look_image(
    ...
    if cached:
        ms = int((time.perf_counter() - t0) * 1000)
        print(f"[timing] model-look cache=1 key={key[-24:]} duration_ms={ms}", flush=True)
        return cached[0].get("image_url")
    if not AI_TEST_MODE and (
    ...
    try:
        ...
        ms = int((time.perf_counter() - t0) * 1000)
        print(f"[model-look] skip after {ms}ms: {exc}", flush=True)
        return None
...
def _apply_model_looks(
    ...
    def one(outfit: dict[str, Any], identity: bytes | None) -> tuple[dict[str, Any], str | None]:
        ...
        if wish and wish.get("storage_path"):
            members.append({
                "id": f"wish-{outfit.get('id') or 'x'}",
                "name": wish.get("name") or "",
                "category": wish.get("category") or "",
...
def live_coordinate(body: LiveCoordinate, user: UserContext = Depends(current_user)):
    ...
    def work(report) -> dict[str, Any]:
        ...
        _record_recommendation_timing(
            user.id, len(pool), len(outfits), recommend_ms,
            {"recommend_ms": recommend_ms, "model_look": int(bool(body.model_look)), "stream": 1},
        )
        if outfits:
